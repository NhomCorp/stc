...
function doPost(e) {
  ...
  if (contents.callback_query) {
    ...
    if (data.startsWith("DELETE_")) {
      const uniqueKey = data.replace("DELETE_", "");
      const isFound = deleteLogRowByUniqueKey(uniqueKey);
      const text = isFound
        ? `🗑️ <b>Đã xoá giao dịch</b>\n└ Mã: <code>${escapeHtml(uniqueKey)}</code>`
        : `❌ <b>Không tìm thấy GD</b> <code>${escapeHtml(uniqueKey)}</code>`;
      editMessage(chatId, messageId, text, { inline_keyboard: [] });
      return;
    }
  ...
  if (!mustFix) {
    payload.reply_markup = {
      inline_keyboard: [[
        ...
        { text: "❌ Xoá", callback_data: `DELETE_${uniqueKey}` }
      ]]
    };
  }
...
/** Xoá dòng Log theo uniqueKey */
function deleteLogRowByUniqueKey(uniqueKey) {
  try {
    const found = findLogRowsByUniqueKey(uniqueKey);
    if (!found || found.rows.length === 0) return false;
    // Xoá từ dưới lên để không lệch index
    found.rows.sort(function(a, b) { return b - a; }).forEach(function(row) {
